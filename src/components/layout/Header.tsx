...
import { Menu, X, Phone, Mail, ChevronDown, Ship, Sparkles, Sun, FerrisWheel, Waves, MapPin, Search, ArrowRight, Car, Building, FileText } from "lucide-react";
...
const Header = () => {
  ...
  const serviceCategories = [{
    ...
    path: "/combo-packages",
    icon: Sparkles,
    description: "Bundled tour packages"
  }, {
    name: "Car Rentals",
    path: "/car-rentals",
    icon: Car,
    description: "Luxury & economy vehicles"
  }, {
    name: "Hotels",
    path: "/hotels",
    icon: Building,
    ...
    path: "/visa-services",
    icon: FileText,
    description: "UAE visa assistance"
  }];
  ...
  const renderDropdown = (type: DropdownType) => {
    if (type === "activities") {
      return <div className="bg-card rounded-2xl shadow-2xl border border-border overflow-hidden">
          <div className="grid grid-cols-3 gap-0">
            <div className="col-span-2 p-6 border-r border-border">
              ...
              <div className="grid grid-cols-2 gap-2">
                {activityCategories.map(category => <Link key={category.path} to={category.path} className="flex items-start gap-3 p-3 rounded-xl hover:bg-muted/50 transition-all group">
            ...
            <div className="p-6 bg-muted/30">
              ...
              <div className="space-y-3">
                {featuredActivities.map(activity => <Link key={activity.path} to={activity.path} className="block group">
              ...
              <Link to="/experiences" className="flex items-center gap-2 text-sm font-semibold text-secondary mt-4 hover:underline">
                View All Activities
    ...
    if (type === "services") {
      return <div className="bg-card rounded-xl shadow-2xl border border-border overflow-hidden p-5 w-[320px]">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
            Our Services
          </p>
          <div className="space-y-1">
            {serviceCategories.map(category => <Link key={category.path} to={category.path} className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/50 transition-all group">
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-background/98 backdrop-blur-lg shadow-lg" : "bg-background/95 backdrop-blur-md"} border-b border-border`}>
      {/* Top bar */}
      ...
      <nav className="container py-3 lg:py-4">
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center group">
            <motion.img src={betterviewLogo} alt="Betterview Tourism" className={cn("object-contain rounded-lg transition-all duration-300", isScrolled ? "h-10 w-auto" : "h-12 w-auto")} />
          </Link>

          {/* Desktop Navigation - Clean & Simple */}
          <div className="hidden lg:flex items-center gap-2">
            {navLinks.map(link => link.dropdown ? <div key={link.path} className="relative" onMouseEnter={() => setActiveDropdown(link.dropdown)} onMouseLeave={() => setActiveDropdown(null)}>
                  <button className={cn("flex items-center gap-1.5 text-sm font-medium px-4 py-2.5 rounded-full transition-all duration-200", isActive(link.path) ? "text-secondary bg-secondary/10" : "text-foreground/80 hover:text-foreground hover:bg-muted")}>
                    {link.name}
                    <ChevronDown className={cn("w-3.5 h-3.5 transition-transform duration-200", activeDropdown === link.dropdown && "rotate-180")} />
                  </button>

                  {/* Dropdown Menu */}
                  <AnimatePresence>
                    {activeDropdown === link.dropdown && <motion.div initial={{
                opacity: 0,
                y: 8
              }} animate={{
                opacity: 1,
                ...
                duration: 0.15
              }} className={cn("absolute top-full pt-3", link.dropdown === "activities" ? "left-1/2 -translate-x-1/2 w-[700px]" : "left-0")}>
                        {renderDropdown(link.dropdown)}
                      </motion.div>}
                  </AnimatePresence>
                </div> : <Link key={link.path} to={link.path} className={cn('text-sm font-medium transition-all duration-200 px-4 py-2.5 rounded-full', link.highlight ? 'bg-secondary text-secondary-foreground hover:bg-secondary/90' : isActive(link.path) ? 'text-secondary bg-secondary/10' : 'text-foreground/80 hover:text-foreground hover:bg-muted')}>
                  {link.name}
                </Link>)}
          </div>
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <div className="relative">
              <AnimatePresence>
                ...
                duration: 0.2
              }} onSubmit={handleSearch} className="overflow-hidden">
                    <Input type="search" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="h-9 pr-9 rounded-full bg-muted/50 border-0 focus-visible:ring-1" autoFocus onBlur={() => {
                  ...
                  </motion.form> : <Button variant="ghost" size="icon" onClick={() => setIsSearchOpen(true)} className="text-foreground/70 hover:text-foreground hover:bg-muted rounded-full h-9 w-9">
          ...
          {/* Mobile Menu Button */}
          <button className="lg:hidden p-2 rounded-full hover:bg-muted transition-colors" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle menu">
      ...
      {createPortal(<AnimatePresence>
        ...
        duration: 0.2
              ...
              <div className="container py-6 flex flex-col min-h-full">
                ...
                <form onSubmit={handleSearch} className="mb-8">
                  <div className="relative">
                    ...
                    <Input type="search" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="h-11 pl-11 text-sm rounded-full bg-muted/50 border-0 focus-visible:ring-1" />
                  </div>
                </form>
              ...
              delay: index * 0.03
                      ...
                      <Link to={link.path} className={cn("flex items-center justify-between text-base font-medium py-3.5 px-4 rounded-xl transition-colors", isActive(link.path) ? "text-secondary bg-secondary/10" : "text-foreground/80 hover:text-foreground hover:bg-muted")} onClick={() => setIsMenuOpen(false)}>
                        {link.name}
                        {link.highlight && <span className="text-xs bg-secondary text-secondary-foreground px-2.5 py-1 rounded-full font-medium">
                            AI
                          </span>}
                      </Link>
                    </motion.div>)}
                ...
                <div className="border-t border-border/50 pt-6 mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {activityCategories.slice(0, 6).map((category, index) => <motion.div key={category.path} initial={{
                opacity: 0,
                scale: 0.95
              }} animate={{
                opacity: 1,
                ...
                delay: 0.15 + index * 0.03
                        ...
                        <Link to={category.path} onClick={() => setIsMenuOpen(false)} className="flex flex-col items-center gap-2 p-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors text-center">
                ...
                <div className="mt-auto pt-4 space-y-3 pb-safe">
                  <a href={`tel:${phone}`} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors">
