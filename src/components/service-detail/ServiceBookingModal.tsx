...
const ServiceBookingModal = ({ 
  ...
  const calculateSubtotal = () => {
    switch (service.bookingType) {
      case "per_person":
        return service.price * adults + service.price * 0.5 * children;
      case "per_group":
        return service.price;
      case "per_vehicle":
        return service.price;
      default:
        return service.price * adults;
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const bookingData: any = {
        tour_id: service.slug,
        tour_name: service.title,
        service_id: service.id,
        booking_source: "service",
        booking_date: format(date!, "yyyy-MM-dd"),
        adults: service.bookingType === "per_person" ? adults : (service.bookingType === "per_group" ? groupSize : 1),
        children: service.bookingType === "per_person" ? children : 0,
        infants: service.bookingType === "per_person" ? infants : 0,
  ...
  const getBookingTypeIcon = () => {
    switch (service.bookingType) {
      case "per_vehicle": return <Car className="w-6 h-6 text-secondary" />;
      default: return <Users className="w-6 h-6 text-secondary" />;
    }
  };
          ...
          <div className={cn(
            ...
            {currentStep === 1 && (
              <div className="space-y-5 sm:space-y-6">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">Choose Your Date</h2>
                  <p className="text-sm sm:text-base text-muted-foreground mt-1">{service.title}</p>
                </div>
                ...
                <div className="flex items-center gap-3 p-4 bg-secondary/10 border border-secondary/30 rounded-2xl">
                  ...
                  <div>
                    <p className="font-bold text-foreground">
                      {service.bookingType === "per_vehicle" ? "Private Transfer" : 
                       service.bookingType === "per_group" ? "Group Experience" : "Per Person Pricing"}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      AED {service.price.toLocaleString()} {getBookingTypeLabel()}
                    </p>
                  </div>
                </div>
                ...
                {/* Guest Counters - Varies by booking type */}
                {service.bookingType === "per_person" && (
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                      <GuestCounter
                        ...
                        onChange={setAdults}
                        min={1}
                        max={service.maxParticipants || 20}
                      />
                      <GuestCounter
                        label="Children"
                ...
                {service.bookingType === "per_group" && (
                  <div>
                    <label className="text-sm font-bold text-foreground mb-3 block">Group Size *</label>
                    <GuestCounter
                      label="Guests"
                      sublabel={`Up to ${service.maxParticipants || 10} people`}
                      value={groupSize}
                      onChange={setGroupSize}
                      min={1}
                      max={service.maxParticipants || 10}
                    />
                  </div>
                )}
                ...
                <div className="bg-muted/30 border border-border rounded-2xl p-4 sm:p-5 flex items-center justify-between">
                  <span className="text-sm sm:text-base text-muted-foreground font-medium">
                    {service.bookingType === "per_vehicle" ? "Transfer Price" : "Estimated Total"}
                  </span>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      className="min-h-[100px] rounded-xl border-2 border-border focus:border-secondary text-sm sm:text-base resize-none"
                    />
                  </div>
                </div>
              </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-5 sm:space-y-6">
                ...
                {/* Booking Summary */}
                <div className="space-y-4 bg-muted/30 rounded-2xl p-5 sm:p-6 border border-border">
                  ...
                  <div className="space-y-3">
                      ...
                      <>
                        {adults > 0 && (
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">AED {(service.price * adults).toLocaleString()}</span>
                          </div>
                        )}
                        {children > 0 && (
                          <div className="flex justify-between text-sm">
                            <span className="text-muted-foreground">Children × {children} (50% off)</span>
                            <span className="font-medium">AED {(service.price * 0.5 * children).toLocaleString()}</span>
                          </div>
                        )}
                        {infants > 0 && (
                          <div className="flex justify-between text-sm">
                            <span className="text-muted-foreground">Infants × {infants}</span>
                    ...
                    {service.bookingType === "per_group" && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Group ({groupSize} guests)</span>
                        <span className="font-medium">AED {service.price.toLocaleString()}</span>
                      </div>
                    )}

                    {service.bookingType === "per_vehicle" && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Private Transfer</span>
                        <span className="font-medium">AED {service.price.toLocaleString()}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-4 border-t border-border space-y-2">
                    ...
                    <div className="flex justify-between text-xl font-bold pt-2">
                      <span>Total</span>
                      <span className="text-primary">AED {totalPrice.toLocaleString()}</span>
                    </div>
                  </div>
                </div>

                {/* Customer Info Summary */}
                <div className="bg-muted/30 rounded-2xl p-5 border border-border space-y-2">
                  ...
                  <p className="text-sm"><span className="text-muted-foreground">Phone:</span> <span className="font-medium">{phone}</span></p>
                  {specialRequests && (
                    <p className="text-sm"><span className="text-muted-foreground">Requests:</span> <span className="font-medium">{specialRequests}</span></p>
                  )}
                </div>
          ...
          <div className="flex gap-3 mt-8">
            {currentStep > 1 && (
              <Button
                ...
                className="h-12 sm:h-14 px-6 rounded-xl border-2 text-sm sm:text-base font-semibold"
              >
            ...
            {currentStep < 3 ? (
              <Button
                onClick={handleNext}
                className="flex-1 h-12 sm:h-14 bg-secondary hover:bg-secondary/90 text-secondary-foreground rounded-xl text-sm sm:text-base font-semibold shadow-lg shadow-secondary/25"
              >
                Continue
              ...
              <Button
                ...
                disabled={isSubmitting}
                className="flex-1 h-12 sm:h-14 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl text-sm sm:text-base font-bold shadow-lg shadow-primary/25"
              >
                {isSubmitting ? (
                  <>
