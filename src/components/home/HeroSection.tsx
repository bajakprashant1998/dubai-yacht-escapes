...
const LiveRevenueCounter = memo(() => {
  ...
  return <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-card/10 backdrop-blur-sm border border-primary-foreground/20 text-primary-foreground" initial={{
    opacity: 0,
    y: -10
  }} animate={{
    opacity: 1,
    ...
    delay: 0.8
      ...
      <div className="relative flex items-center gap-1.5">
        <motion.div className="w-2 h-2 rounded-full bg-secondary" animate={pulse ? {
        scale: [1, 1.5, 1]
      } : {}} />
        <motion.div className="absolute w-2 h-2 rounded-full bg-secondary" animate={{
        scale: [1, 2.5],
        opacity: [0.6, 0]
      }} transition={{
        duration: 1.5,
...
const HeroSection = memo(() => {
  ...
  const statsDisplay = [{
    ...
    value: stats.support,
    label: stats.supportLabel
  }];
  return <section className="relative min-h-[95vh] flex items-center overflow-visible pb-24">
      {/* Background Video/Image with Parallax */}
      <motion.div className="absolute inset-0" style={{
      y: backgroundY
    }}>
        {/* Video Background */}
        {showVideo && <video ref={videoRef} autoPlay muted loop playsInline preload="metadata" poster={heroBurjKhalifaEvening} onLoadedData={() => setVideoLoaded(true)} onError={() => setVideoError(true)} className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${videoLoaded ? "opacity-100" : "opacity-0"}`}>
            <source src="/assets/dubai-hero-video.mp4" type="video/mp4" />
          </video>}
        
        {/* Fallback Image with Ken Burns effect */}
        <motion.div className={`absolute inset-0 transition-opacity duration-1000 ${showVideo && videoLoaded ? "opacity-0" : "opacity-100"}`} initial={{
        scale: 1.1
      }} animate={{
        scale: 1
      }} transition={{
        ...
        repeatType: "reverse"
          ...
          <OptimizedImage src={heroBurjKhalifaEvening} alt="Dubai skyline with Burj Khalifa at sunset evening view" priority objectFit="cover" sizes="100vw" onLoad={() => setImageLoaded(true)} containerClassName="w-full h-full" />
        </motion.div>
        ...
        <motion.div className="absolute inset-0 bg-gradient-to-r from-secondary/10 via-transparent to-secondary/5" animate={{
        opacity: [0.3, 0.6, 0.3]
      }} transition={{
        duration: 8,
        repeat: Infinity,
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <motion.div className="absolute top-20 right-[15%] w-96 h-96 bg-gradient-radial from-amber-400/20 via-orange-500/10 to-transparent rounded-full blur-3xl" animate={{
        y: [0, -40, 0],
        x: [0, 20, 0],
        scale: [1, 1.2, 1],
        opacity: [0.4, 0.7, 0.4]
        ...
        <motion.div className="absolute bottom-32 left-[8%] w-72 h-72 bg-gradient-radial from-purple-500/15 via-violet-600/8 to-transparent rounded-full blur-2xl" animate={{
        y: [0, 30, 0],
        x: [0, -15, 0],
        scale: [1, 1.15, 1],
        opacity: [0.3, 0.6, 0.3]
        ...
        <motion.div className="absolute top-1/2 right-[25%] w-48 h-48 bg-gradient-radial from-orange-400/15 via-amber-500/8 to-transparent rounded-full blur-xl" animate={{
        y: [0, -25, 0],
        scale: [1, 1.3, 1],
        opacity: [0.2, 0.5, 0.2]
      }} transition={{
        ...
        <motion.div className="absolute top-[30%] left-[20%] w-2 h-2 bg-secondary/60 rounded-full" animate={{
        y: [0, -20, 0],
        opacity: [0, 1, 0],
        scale: [0.5, 1, 0.5]
      }} transition={{
        ...
        <motion.div className="absolute top-[60%] right-[35%] w-3 h-3 bg-amber-300/50 rounded-full" animate={{
        y: [0, -30, 0],
        opacity: [0, 1, 0],
        scale: [0.5, 1.2, 0.5]
      }} transition={{
        ...
        <motion.div className="absolute top-[45%] left-[60%] w-2 h-2 bg-white/40 rounded-full" animate={{
        y: [0, -15, 0],
        opacity: [0, 0.8, 0],
        scale: [0.5, 1, 0.5]
      }} transition={{
      ...
      <motion.div className="container relative z-10 py-20" style={{
      y: contentY
    }}>
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto">
          <motion.div className="text-primary-foreground" variants={containerVariants} initial="hidden" animate="visible">
            ...
            {/* Badge */}
            <motion.div className="inline-flex items-center gap-2 bg-secondary/20 backdrop-blur-sm text-secondary px-4 py-2 rounded-full mb-6 border border-secondary/30" variants={itemVariants}>
              <motion.div animate={{
              rotate: [0, 15, -15, 0]
            }} transition={{
              duration: 2,
            ...
            <motion.h1 className="font-display text-fluid-3xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-[1.05] mb-4" variants={itemVariants}>
              Discover Dubai's
              <motion.span className="block text-shimmer mt-2" animate={{
            ...
            <motion.div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 w-full" variants={itemVariants}>
              {statsDisplay.map((stat, index) => <motion.div key={index} className="text-center bg-primary-foreground/5 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-primary-foreground/10 cursor-default" whileHover={{
              scale: 1.05,
              y: -5,
              ...
              damping: 20
                  ...
                  <motion.p className="text-2xl sm:text-3xl lg:text-4xl font-bold text-secondary" initial={{
                opacity: 0,
                scale: 0.5
              }} animate={{
                opacity: 1,
      ...
      <div className="absolute left-0 right-0 z-20 px-3 md:px-4" style={{
      bottom: '-80px'
    }}>
        <div className="container mx-auto">
          <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-8 gap-1.5 sm:gap-2 md:gap-3">
            ...
            overlay: "from-amber-600/80 to-orange-500/60",
            slug: "desert-safari"
          }, {
            icon: Ticket,
            label: "Theme Parks",
            image: themeParksImg,
            overlay: "from-rose-600/80 to-pink-500/60",
            slug: "theme-parks"
          }, {
            icon: Waves,
            label: "Water Sports",
            image: waterSportsImg,
            overlay: "from-cyan-600/80 to-blue-500/60",
            slug: "water-sports"
          }, {
            icon: Ship,
            label: "Cruises",
            image: dhowCruiseImg,
            overlay: "from-blue-600/80 to-indigo-500/60",
            slug: "sightseeing-cruises"
          }, {
            icon: Mountain,
            label: "Adventure",
            image: adventureImg,
            overlay: "from-violet-600/80 to-purple-500/60",
            slug: "adventure-sports"
          }, {
            icon: MapPin,
            label: "City Tours",
            image: cityToursImg,
            overlay: "from-orange-600/80 to-amber-500/60",
            slug: "city-tours"
          }, {
            icon: Eye,
            label: "Observation",
            image: observationImg,
            overlay: "from-emerald-600/80 to-teal-500/60",
            slug: "observation-decks"
          }, {
            icon: Utensils,
            label: "Dining",
            image: diningImg,
            overlay: "from-pink-600/80 to-rose-500/60",
            slug: "dining-experiences"
          }].map((item, index) => <Link key={item.label} to={`/experiences?category=${item.slug}`} className="block h-full">
                <motion.div initial={{
              opacity: 0,
              y: 30,
              scale: 0.9
              ...
              scale: 0.98
                  ...
                  <div className="relative z-10 h-full flex flex-col items-center justify-center p-2 sm:p-3 md:p-4 text-center">
                    <motion.div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-lg sm:rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center mb-1.5 sm:mb-2 md:mb-3 border border-white/30" whileHover={{
                  rotate: [0, -10, 10, 0]
                }} transition={{
                  duration: 0.5
                }}>
